...
namespace DataLayer.MySql {
    ...
    public class MySqlDatabase : Database {
        ...
        protected override async Task OpenAsync(IDbConnection connection) {
            await ((MySqlConnection)connection).OpenAsync().ConfigureAwait(false);
        }

        /// <inheritdoc/>
        protected override async Task<IDataReader> ExecuteReaderCoreAsync(IDbCommand command) {
            return await ((MySqlCommand)command).ExecuteReaderAsync(CommandBehavior.CloseConnection).ConfigureAwait(false);
        }

        /// <inheritdoc/>
        protected override async Task<int> ExecuteNonQueryCoreAsync(IDbCommand command) {
            return await ((MySqlCommand)command).ExecuteNonQueryAsync().ConfigureAwait(false);
        }

        /// <inheritdoc/>
        protected override async Task<object?> ExecuteScalarCoreAsync(IDbCommand command) {
            return await ((MySqlCommand)command).ExecuteScalarAsync().ConfigureAwait(false);
        }
    }
}
